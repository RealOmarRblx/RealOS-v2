...
const Storage = {
    ...
    saveSettings: () => {
        const data = {
            ...
            systemWideAnimSpeed: State.systemWideAnimSpeed
        };
...
const State = {
    ...
    systemWideAnimSpeed: localStorage.getItem('realos_sys_anim') === 'true',
...
const OS = {
    ...
    updateTime: () => {
        ...
        if (State.aod.enabled && !State.poweredOn && State.systemWideAnimSpeed) {
    ...
    setupGestures: () => {
        ...
        newBar.addEventListener('touchstart', e => {
            ...
            isSwipe = false;
            newBar.style.transform = 'scale(0.95)';
        }, { passive: false });

        newBar.addEventListener('touchmove', e => {
            if (!startY) return;
            const diff = e.touches[0].clientY - startY;
            if (diff < -10) isSwipe = true;
        }, { passive: false });

        newBar.addEventListener('touchend', e => {
            newBar.style.transform = 'scale(1)';
            if (State.swipeToClose) {
                if (startY && e.changedTouches[0].clientY - startY < -40) {
                    AppManager.close();
                }
            } else {
                if (!isSwipe) handleClose();
            }
            startY = 0;
        });
        ...
        newBar.addEventListener('mouseup', e => {
            newBar.style.transform = 'scale(1)';
            if (State.swipeToClose) {
                if (startY && e.clientY - startY < -40) {
                    AppManager.close();
                }
            } else {

                if (Math.abs(e.clientY - startY) < 5) handleClose();
            }
            startY = 0;
        });
    ...
    applySettings: () => {
        ...
        const animDur = State.systemWideAnimSpeed ? (0.5 * State.animationSpeed) : 0.5;
...
const AppManager = {
    ...
    open: (id) => {
        ...
        if (id === AppManager.closingAppId && AppManager.closingApp) {
            ...
            const packColor = State.iconPack === 'hyperos' ? appInfo.hyperColor : (State.iconPack === 'coloros' ? appInfo.colorColor : null);
            if ((State.iconPack === 'hyperos' || State.iconPack === 'coloros') && packColor) {
                win.style.background = 'transparent';
            } else {
                win.style.setProperty('background', appInfo.color || '#000', 'important');
            }

            const cloneIcon = closeClone.querySelector('i') || closeClone.querySelector('img');
            const isImg = cloneIcon && cloneIcon.tagName === 'IMG';
            const cloneIconRect = cloneIcon ? cloneIcon.getBoundingClientRect() : null;
            const cloneIconOpacity = cloneIcon ? window.getComputedStyle(cloneIcon).opacity : '1';

            let iconStartStyle = '';
            if (isImg) {
                const cloneW = cloneIcon ? window.getComputedStyle(cloneIcon).width : '64px';
                const parsedW = parseFloat(cloneW) || 64;
                iconStartStyle = `width: ${Math.max(parsedW, 64)}px; height: ${Math.max(parsedW, 64)}px; object-fit: cover; border-radius: inherit;`;
            } else {
                const cloneIconFontSize = cloneIcon ? window.getComputedStyle(cloneIcon).fontSize : '28px';
                const parsedSize = parseFloat(cloneIconFontSize) || 40;
                iconStartStyle = `font-size: ${Math.max(parsedSize, 40)}px;`;
            }
            ...
            if (isImagePack) {
                ...
                iconEl2 = document.createElement('img');
                iconEl2.src = packIcon;
                iconEl2.style.cssText = `
                        ...
                        border-radius: inherit;
                        transform: scale(1.2);
                        opacity: ${cloneIconOpacity};
                        transition: opacity ${0.1 * State.animationSpeed}s ease;
                `;
            } else {
                iconEl2 = document.createElement('i');
                iconEl2.className = `fas ${appInfo.icon}`;
                iconEl2.style.cssText = `
                        position: absolute;
                        top: 50%;
                        left: 50%;
                        ...
                        ${iconStartStyle}
                        color: ${appInfo.text || 'white'};
                        opacity: ${cloneIconOpacity};
                        transition: font-size ${0.3 * State.animationSpeed}s ease, opacity ${0.25 * State.animationSpeed}s ease;
                `;
            }

            if (iconEl2) {
                iconOverlay.appendChild(iconEl2);
                ...
                requestAnimationFrame(() => {
                    ...
                    if (iconOverlay) {
                        ...
                        if (overlayIcon) {
                            ...
                            overlayIcon.style.transition = `all ${0.5 * State.animationSpeed}s cubic-bezier(0.32, 0.72, 0, 1), opacity ${0.25 * State.animationSpeed}s ease`;
                    ...
                    setTimeout(() => {
                        if (overlayIcon) {
                            overlayIcon.style.transition = `font-size ${0.3 * State.animationSpeed}s ease, opacity ${0.25 * State.animationSpeed}s ease`;
        ...
        if (State.activeApp && !AppManager.closingApp) {
            AppManager.close();
        ...
        iconOverlay.style.cssText = `
                    ...
                    pointer-events: none;
                    border-radius: inherit;
                    transition: opacity ${0.15 * State.animationSpeed}s ease;
                `;
        ...
        if (isImagePackNorm) {
            ...
            iconOverlay.innerHTML = `<img src="${packIconNorm}" style="width: 100%; height: 100%; object-fit: cover; border-radius: inherit; transform: scale(1.2); transition: opacity ${0.1 * State.animationSpeed}s ease;">`;
        } else {
            iconOverlay.innerHTML = `<i class="fas ${appInfo.icon}" style="font-size: 28px; color: ${appInfo.text || 'white'}; transition: font-size ${0.5 * State.animationSpeed}s cubic-bezier(0.32, 0.72, 0, 1);"></i>`;
        }
        win.appendChild(iconOverlay);
        ...
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                ...
                setTimeout(() => {
                    if (iconOverlay) iconOverlay.style.opacity = '0';
                    header.style.transition = `opacity ${0.3 * State.animationSpeed}s ease`;
                    appBody.style.transition = `opacity ${0.3 * State.animationSpeed}s ease`;
                    appBody.style.opacity = '1';
                    header.style.opacity = '1';
    ...
    close: () => {
        ...
        const ghostModal = closeClone.querySelector('#new-note-modal');
        if (ghostModal) ghostModal.remove();
        ...
        iconLayer.style.transition = `opacity ${0.12 * State.animationSpeed}s ease`;
        iconLayer.style.zIndex = '999';
        ...
        if (isImagePackClose) {
            ...
            iconInner.style.cssText = `width: 100%; height: 100%; object-fit: cover; border-radius: inherit; transform: scale(1.2); transition: transform ${0.45 * State.animationSpeed}s cubic-bezier(0.32, 0.72, 0, 1);`;
        } else {
            ...
            iconInner.style.cssText = `font-size: 50px; color: ${appInfo.text || 'white'}; transition: font-size ${0.45 * State.animationSpeed}s cubic-bezier(0.32, 0.72, 0, 1);`;
        }
        ...
        const transTime = 0.45 * State.animationSpeed;

        const baseTrans = `transform ${transTime}s cubic-bezier(0.32, 0.72, 0, 1), width ${transTime}s cubic-bezier(0.32, 0.72, 0, 1), height ${transTime}s cubic-bezier(0.32, 0.72, 0, 1), border-radius ${transTime}s cubic-bezier(0.32, 0.72, 0, 1)`;
        ...
        if (isImagePackClose && iconInner) {
            iconInner.style.transform = 'scale(1.0)';
        } else if (iconInner) {
            iconInner.style.fontSize = '28px';
        }

        let finalRadius = '16px';
        let clipPath = 'inset(0 round 16px)';
        if (State.appShape === 'circle') {
            finalRadius = '50%';
            clipPath = 'inset(0 round 50%)';
        } else if (State.appShape === 'squircle') {
            finalRadius = (endW * 0.22) + 'px';
            clipPath = `inset(0 round ${finalRadius})`;
        }
...
const Island = {
    ...
    tapMain: (e) => {
        ...
        else if (Music.active && !Music.audio.paused) Island.expand('music');
...
const Apps = {
    ...
    settings: {
        ...
        render: (v) => {
            ...
            if (view === 'root') {
                ...
                content = `<div class="anim-fade">
                            ...
                            <div style="padding:0 20px 5px; font-size:13px; color:var(--text-sec); margin-top:10px;">GESTURES & SYSTEM</div>
                            <div class="list-group">
                                ...
                                <div class="list-item" onclick="Apps.settings.toggleSysAnim()">
                                    <span>Apply Speed System-Wide</span>
                                    <div class="toggle ${State.systemWideAnimSpeed ? 'active' : ''}"></div>
                                </div>
                            </div>

                            <div style="padding:0 20px 5px; font-size:13px; color:var(--text-sec); margin-top:10px;">ICON PACK</div>
                            <!-- Preview Section -->
                            <div style="margin:0 20px 10px; padding:15px; background:var(--bg-card); border-radius:12px; display:flex; justify-content:center; gap:20px; align-items:center;">
                                ${(() => {
                        const isHyper = State.iconPack === 'hyperos';
                        const isColor = State.iconPack === 'coloros';
                        const isImagePack = isHyper || isColor;
        ...
        togglePunch: () => { State.punchHole = !State.punchHole; OS.applySettings(); Apps.settings.render('island'); localStorage.setItem('realos_punch', State.punchHole); },
        toggleMusicGrad: () => { State.musicGradient = !State.musicGradient; Storage.saveSettings(); Apps.settings.render('island'); Island.update(); localStorage.setItem('realos_music_grad', State.musicGradient); },
        setAppShape: (shape) => {
            State.appShape = shape;
            OS.applySettings();
            Apps.settings.render('customization');
        ...
        toggleSysAnim: () => {
            State.systemWideAnimSpeed = !State.systemWideAnimSpeed;
            OS.applySettings();
            localStorage.setItem('realos_sys_anim', State.systemWideAnimSpeed);
            Storage.saveSettings();
            Apps.settings.render('customization');
        },
