    ...
    loadSettings: () => {
        ...
        if (data) {
            ...
            if (parsed.wallpapers) {
                ...
                if (!JSON.parse(data).wallpapers.includes(defaultWallpapers[0])) {
...
const State = {
    ...
    tapIndicators: false,
    brightness: 100,
    wallpapers: ['https://i.ibb.co/7tFYztFD/waves-purple-digital-abstract-top-best-trending-desktop-wallpapers-backgrounds-and-screensavers-for.png', 'https://i.ibb.co/jP4DGMYR/wallpaper.jpg', 'https://i.ibb.co/9HGWgS4w/wallpaper3.jpg', 'https://i.ibb.co/FMtRmsm/wallpaper4.png', 'https://i.ibb.co/ymJxLsYz/wallpaper5.png', 'https://i.ibb.co/43v4xw9/wallpaper6.png'],
    currentWall: 0,
    poweredOn: true,
    security: {
        pin: null,
...
const OS = {
    ...
    updateWallpaperEffect: () => {
        ...
        const isSpecial = specialUrls.includes(currentUrl) && State.specialEffects;
        ...
        if (isSpecial) {
            ...
            if (!State.poweredOn && State.aod.enabled) {
                ...
                setTimeout(() => {
                    if (!State.locked && specialUrls.includes(currentUrl)) {
                        wallLayer.style.transition = '';
                    }
                }, 1000);
            }

            if (State.aod.enabled && State.aod.wallpaper) {
    ...
    settings: {
        ...
        render: (v) => {
            ...
            if (view === 'root') {
                ...
                State.wallpapers.forEach((url, i) => {
                    const isSpecialWall = specialUrls.includes(url);
                    const sparkleIcon = isSpecialWall ? `
                        <svg class="sparkle-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <defs>
                                <linearGradient id="sparkle-grad-${i}" x1="0%" y1="0%" x2="100%" y2="100%">
                                    <stop offset="0%" class="sparkle-stop1" stop-color="#9b59b6"/>
                                    <stop offset="100%" class="sparkle-stop2" stop-color="#1a1a2e"/>
                                </linearGradient>
                            </defs>
                            <path d="M12 2L14.5 9.5L22 12L14.5 14.5L12 22L9.5 14.5L2 12L9.5 9.5L12 2Z" fill="url(#sparkle-grad-${i})" stroke="rgba(255,255,255,0.5)" stroke-width="0.5"/>
                        </svg>` : '';
                    gridItems += `
                                 <div class="wall-grid-item ${i === State.currentWall ? 'active' : ''}" onclick="Apps.settings.setWall(${i})" style="position:relative; min-width:70px; width:70px; height:140px; border-radius:8px; overflow:hidden; border:2px solid ${i === State.currentWall ? 'var(--accent)' : 'transparent'}; cursor:pointer; flex-shrink:0;">
                                     ${sparkleIcon}
                ...
                gridItems += `
                             <div class="wall-grid-item" onclick="document.getElementById('wall-input').click()" style="min-width:70px; width:70px; height:140px; border-radius:8px; overflow:hidden; border:2px dashed #444; display:flex; align-items:center; justify-content:center; cursor:pointer; flex-shrink:0;">
                ...
                content = `
                        <div class="anim-fade" style="padding:20px; height:100%; display:flex; flex-direction:column; box-sizing:border-box;">
                             ...
                             <div style="margin-bottom:20px; display:flex; gap:10px; overflow-x:auto; padding-bottom:5px; flex-wrap:nowrap;">
                                 ${gridItems}
                             </div>
                             <div class="list-group" style="margin:0;">
                                ...
                                <div id="special-effects-item" class="list-item ${['https://i.ibb.co/9HGWgS4w/wallpaper3.jpg', 'https://i.ibb.co/FMtRmsm/wallpaper4.png', 'https://i.ibb.co/ymJxLsYz/wallpaper5.png', 'https://i.ibb.co/43v4xw9/wallpaper6.png'].includes(State.wallpapers[State.currentWall]) ? '' : 'disabled'}" style="${['https://i.ibb.co/9HGWgS4w/wallpaper3.jpg', 'https://i.ibb.co/FMtRmsm/wallpaper4.png', 'https://i.ibb.co/ymJxLsYz/wallpaper5.png', 'https://i.ibb.co/43v4xw9/wallpaper6.png'].includes(State.wallpapers[State.currentWall]) ? '' : 'opacity:0.5; pointer-events:none;'}" onclick="Apps.settings.toggleSpecialEffects()">
                                    <span>Special Effects</span>
                                    <div class="toggle ${State.specialEffects ? 'active' : ''}"></div>
                                </div>
                                <div id="special-effects-desc" style="margin:10px 20px; font-size:12px; color:var(--text-sec); display:${['https://i.ibb.co/9HGWgS4w/wallpaper3.jpg', 'https://i.ibb.co/FMtRmsm/wallpaper4.png', 'https://i.ibb.co/ymJxLsYz/wallpaper5.png', 'https://i.ibb.co/43v4xw9/wallpaper6.png'].includes(State.wallpapers[State.currentWall]) ? 'block' : 'none'}">Special Effects for the wallpaper you selected is supported</div>
                            </div>
                        </div>`;
            }
                ...
                content = `<div class="anim-fade" style="padding-top:90px; position:relative; min-height:100%;">
                            ...
                            <div class="about-hero" style="z-index:2; position:relative;">
                                ...
                                <div class="realos-ver">V2.2.5</div>
                            </div>
        ...
        setWall: (i) => {
            ...
            document.querySelectorAll('.wall-grid-item').forEach((item, idx) => {
                ...
                item.style.borderColor = isSelected ? 'var(--accent)' : 'transparent';
            ...
            const specialUrls = ['https://i.ibb.co/9HGWgS4w/wallpaper3.jpg', 'https://i.ibb.co/FMtRmsm/wallpaper4.png', 'https://i.ibb.co/ymJxLsYz/wallpaper5.png', 'https://i.ibb.co/43v4xw9/wallpaper6.png'];
            const isSpecial = specialUrls.includes(State.wallpapers[i]);

            const specialItem = document.getElementById('special-effects-item');

            if (specialItem) {
                if (isSpecial) {
                    specialItem.classList.remove('disabled');
                    specialItem.style.opacity = '1';
                    specialItem.style.pointerEvents = 'auto';
                } else {
                    specialItem.classList.add('disabled');
                    specialItem.style.opacity = '0.5';
                    specialItem.style.pointerEvents = 'none';
                }
            }

            const note = document.getElementById('special-effects-desc');
            if (note) {
                note.style.display = isSpecial ? 'block' : 'none';
            }
        },
        toggleAOD: () => {
