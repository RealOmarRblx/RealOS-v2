    ...
    loadSettings: () => {
        ...
        if (data) {
            ...
            const defaultWallpapers = [
                'https://i.ibb.co/7tFYztFD/waves-purple-digital-abstract-top-best-trending-desktop-wallpapers-backgrounds-and-screensavers-for.png',
                'https://i.ibb.co/jP4DGMYR/wallpaper.jpg',
...
const State = {
    ...
    tapIndicators: false,
    brightness: 100,
    wallpapers: ['https://i.ibb.co/7tFYztFD/waves-purple-digital-abstract-top-best-trending-desktop-wallpapers-backgrounds-and-screensavers-for.png', 'https://i.ibb.co/jP4DGMYR/wallpaper.jpg', 'https://i.ibb.co/9HGWgS4w/wallpaper3.jpg', 'https://i.ibb.co/FMtRmsm/wallpaper4.png', 'https://i.ibb.co/ymJxLsYz/wallpaper5.png', 'https://i.ibb.co/43v4xw9/wallpaper6.png', 'wallpapers/Xiaomi/xiaomi blue.mp4', 'wallpapers/Xiaomi/xiaomi orange.mp4', 'wallpapers/Xiaomi/xiaomi purple.mp4', 'wallpapers/Xiaomi/xiaomi ultra blue.png', 'wallpapers/Xiaomi/xiaomi ultra green.png', 'wallpapers/Xiaomi/xiaomi ultra orange.png', 'wallpapers/Xiaomi/xiaomi ultra purple.png', 'wallpapers/OriginOS/origin wallpaper dark.png', 'wallpapers/OriginOS/origin wallpaper.png', 'wallpapers/OriginOS/origin wallpaper2.png', 'wallpapers/OriginOS/origin wallpaper3.png'],
    currentWall: 0,
    poweredOn: true,
    security: {
        pin: null,
...
const OS = {
    ...
    updateWallpaperEffect: (idx) => {
        ...
        const toggleUrl = State.wallpapers[i];

        const xiaomiToggle = document.getElementById('toggle-XiaomiWallpapers');
        const specialToggle = document.getElementById('toggle-SpecialWallpapers');

        if (xiaomiToggle) {
            xiaomiToggle.style.maxHeight = '0';
            xiaomiToggle.style.opacity = '0';
        }
        if (specialToggle) {
            specialToggle.style.maxHeight = '0';
            specialToggle.style.opacity = '0';
        }

        const isToggleVideo = isVideoWallpaper(toggleUrl);
        const isToggleXiaomi = toggleUrl && toggleUrl.includes('xiaomi');

        if (toggleUrl && !isToggleVideo && !isToggleXiaomi) {
            if (specialToggle) {
                specialToggle.style.maxHeight = '100px';
                specialToggle.style.opacity = '1';
                specialToggle.style.pointerEvents = 'auto';
                specialToggle.style.filter = 'none';
            }
        } else if (isToggleVideo || isToggleXiaomi) {
            if (xiaomiToggle) {
                xiaomiToggle.style.maxHeight = '100px';
                xiaomiToggle.style.opacity = '1';
                const isVid = isVideoWallpaper(toggleUrl);
                xiaomiToggle.style.pointerEvents = isVid ? 'auto' : 'none';
                xiaomiToggle.style.filter = isVid ? 'none' : 'grayscale(1) opacity(0.5)';
            }
        }
        ...
        if (isSpecial) {
            wallLayer.style.transition = 'background-position 1s cubic-bezier(0.2, 0.8, 0.2, 1), transform 1s cubic-bezier(0.2, 0.8, 0.2, 1)';
        ...
        if (isVideoUrl) {
            ...
            if (VideoWallpaper.videoEl) {
                ...
                v.style.transition = 'transform 1s cubic-bezier(0.2, 0.8, 0.2, 1)';
    ...
    renderApps: () => {
        ...
        APPS.forEach((app, idx) => {
            ...
            if (app.area === 'grid' && State.iconPositions && State.iconPositions[app.id]) {
...
const VideoWallpaper = {
    ...
    reverseToStart: () => {
        ...
        const stepBack = () => {
            ...
            seekStart = now;
            const newTime = video.currentTime - Math.max(delta, 1 / 30);
            if (newTime <= 0) {
                video.currentTime = 0;
                VideoWallpaper.isReversing = false;
                return;
...
const AppManager = {
    ...
    open: (id) => {
        if (id === AppManager.closingAppId && AppManager.closingApp) {
            ...
            if (closeIconLayerOpacity > 0) {
                ...
                iconOverlay.style.cssText = `
                    ...
                    transition: opacity ${0.4 * State.animationSpeed}s ease;
                `;
                ...
                requestAnimationFrame(() => {
                    ...
                    if (State.animStyle === 'new') {
                        ...
                        const anim = win.animate([
                            { top: `${curTop}px`, left: `${curLeft}px`, width: `${curW}px`, height: `${curH}px`, borderRadius: curRadius, backgroundColor: startBg },
                            { backgroundColor: startBg, offset: 0.2 },
                            { backgroundColor: endBg, offset: 0.5 },
                            { top: '0px', left: '0px', width: '100%', height: '100%', borderRadius: '60px', backgroundColor: endBg }
                        ], {
        ...
        if ((State.iconPack === 'hyperos' || State.iconPack === 'coloros') && packColorNorm) {
            ...
            win.style.background = appInfo.color || '#000';
        }
        ...
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                ...
                if (iconOverlay) {
                    iconOverlay.style.transition = `opacity ${totalDur * 0.4}s ease`;
                    iconOverlay.style.opacity = '0';
                }
                win.style.transition = 'none';
                win.style.overflow = 'hidden';
                ...
                if (State.animStyle === 'new') {
                    ...
                    const animation = win.animate([
                        ...
                        {
                            backgroundColor: startBg, offset: 0.35
                        },
                        {
                            backgroundColor: endBg, offset: 0.55
                        },
                        {
                            top: '0px',
                            left: '0px',
    ...
    close: () => {
        ...
        if (id === 'settings' && Apps.settings) Apps.settings.view = 'root';
        if (!State.poweredOn) {
        ...
        const morphToIsland = (id === 'music' && musicPlaying) || (id === 'clock' && timerActive);
        let islandMorphTarget = null;
        let islandTargetEl = null;
        if (morphToIsland) {
            const screenEl = document.getElementById('screen');
            const screenRect = screenEl.getBoundingClientRect();
            const scaleFactor = screenRect.width / screenEl.offsetWidth;
            const island = document.getElementById('dynamic-island');
        ...
        if (!morphToIsland) {
            ...
            if (id === 'music') {
                ...
                miniContentHtml = `<div style="display:flex; align-items:center; width:100%; height:100%; padding:0 20px;">
        ...
        AppManager.closingApp = morphToIsland ? null : closeClone;
        AppManager.closingAppId = morphToIsland ? null : id;
        ...
        if (morphToIsland && islandMorphTarget) {
            ...
            if (iconEl) {
                AppManager.iconFadeTimeout = setTimeout(() => {
                    ...
                    const iconBox = iconEl.querySelector('.icon-box') || iconEl;
                    iconBox.style.transition = `opacity ${totalDur * 0.4}s ease`;
                    iconBox.style.opacity = '1';
                    iconBox.style.visibility = 'visible';
                }, totalDur * 1);
            }
...
const IconDrag = {
    ...
    getGridApps: () => {
        return APPS.filter(a => a.area === 'grid' && !a.hidden);
    },
    ...
    getCellRect: (row, col) => {
        ...
        const gridH = gridRect.height / totalScale;
        const gap = 15;
        const rowGap = 25;
    ...
    findNearestEmpty: (targetRow, targetCol, excludeId) => {
        const occupied = IconDrag.getOccupied(excludeId);
        if (!occupied[`${targetRow},${targetCol}`]) return { row: targetRow, col: targetCol };
        for (let dist = 1; dist <= 8; dist++) {
            for (let dr = -dist; dr <= dist; dr++) {
                for (let dc = -dist; dc <= dist; dc++) {
                    ...
                    if (r < 0 || r >= IconDrag.ROWS || c < 0 || c >= IconDrag.COLS) continue;
    ...
    activate: (icon, pos) => {
        ...
        ph.className = 'grid-placeholder visible';
    ...
    onMove: (e) => {
        ...
        const gridLocalX = (centerX - gridRect.left) / totalScale;
        const gridLocalY = (centerY - gridRect.top) / totalScale;

        const cell = IconDrag.getCellRect(0, 0);
        const targetCol = Math.max(0, Math.min(IconDrag.COLS - 1, Math.floor(gridLocalX / (cell.w + 15))));
        const targetRow = Math.max(0, Math.min(IconDrag.ROWS - 1, Math.floor(gridLocalY / (cell.h + 25))));

        const appId = IconDrag.sourceEl.id.replace('icon-', '');
        const occupied = IconDrag.getOccupied(appId);
        const isOccupied = occupied[`${targetRow},${targetCol}`];

        let destRow, destCol;

        if (isOccupied) {
            const finalPos = IconDrag.findNearestEmpty(targetRow, targetCol, appId);
            destRow = finalPos.row;
            destCol = finalPos.col;
        } else {
            destRow = targetRow;
            destCol = targetCol;
        }
        ...
        if (IconDrag.placeholder) {
            IconDrag.placeholder.style.gridRow = destRow + 1;
            IconDrag.placeholder.style.gridColumn = destCol + 1;
        }
    },
    onEnd: () => {
        ...
        if (IconDrag.placeholder) {
            targetRow = parseInt(IconDrag.placeholder.style.gridRow) - 1;
            targetCol = parseInt(IconDrag.placeholder.style.gridColumn) - 1;
        } else {
    ...
    init: () => {
        Music.audio.onended = () => {
            if (Music.repeat || Music.shuffle) {
                ...
                Music.updateUI();
                if (typeof Island !== 'undefined') Island.update();
                const aodMusic = document.getElementById('aod-music');
                if (aodMusic) aodMusic.style.display = 'none';
            }
        };
...
const Apps = {
    ...
    settings: {
        ...
        render: (v) => {
            ...
            if (view === 'root') {
                ...
                content = `
                    <div class="anim-fade">
                        ...
                        <div class="fp-grid ${!State.security.fingerprint ? 'disabled' : ''}" style="${!State.security.fingerprint ? 'opacity:0.5; pointer-events:none; filter:grayscale(1);' : ''}">
                ...
                headerTitle.innerText = 'Display';
                content = `<div class="anim-fade"><div class="list-group"><div class="list-item" onclick="Apps.settings.toggleDark()"><span>Dark Mode</span><div class="toggle ${State.darkMode ? 'active' : ''}"></div></div><div class="list-item" style="display:block; cursor:default"><div style="margin-bottom:10px; font-size:14px;">Brightness</div><input type="range" min="20" max="100" value="${State.brightness}" oninput="Apps.settings.setBright(this.value)"></div><div class="list-item" onclick="Apps.settings.toggleTap()"><span>Visual Taps</span><div class="toggle ${State.tapIndicators ? 'active' : ''}"></div></div><div class="list-item" onclick="Apps.settings.toggleLiteMode()"><span>Lite Mode</span><div class="toggle ${State.liteMode ? 'active' : ''}"></div></div></div><div style="margin:10px 20px; font-size:12px; color:var(--text-sec);">Lite Mode disables blur effects on Dock, Volume Bar and Pin Pad for better performance.</div></div>`;
            }
            else if (view === 'wallpaper') {
                const savedScrolls = [];
                document.querySelectorAll('.wall-carousel').forEach(c => savedScrolls.push(c.scrollLeft));
                ...
                const defaultUrls = ['https://i.ibb.co/7tFYztFD/waves-purple-digital-abstract-top-best-trending-desktop-wallpapers-backgrounds-and-screensavers-for.png', 'https://i.ibb.co/jP4DGMYR/wallpaper.jpg'];
                ...
                State.wallpapers.forEach((url, i) => {
                    ...
                    if (url.includes('xiaomi')) xiaomiWalls.push({ url, i });
                    else if (url.includes('origin')) originWalls.push({ url, i });
                    else if (specialUrls.includes(url)) specialWalls.push({ url, i });
                    else if (defaultUrls.includes(url)) defaultWalls.push({ url, i });
                    else customWalls.push({ url, i });
                });

                originWalls.splice(4);
                xiaomiWalls.splice(7);

                const sparkle = (idx) => `<svg class="sparkle-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="sparkle-grad-${idx}" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" class="sparkle-stop1" stop-color="#9b59b6"/><stop offset="100%" class="sparkle-stop2" stop-color="#1a1a2e"/></linearGradient></defs><path d="M12 2L14.5 9.5L22 12L14.5 14.5L12 22L9.5 14.5L2 12L9.5 9.5L12 2Z" fill="url(#sparkle-grad-${idx})" stroke="rgba(255,255,255,0.5)" stroke-width="0.5"/></svg>`;
                const wallItem = (url, i, showSparkle) => {
                    const isVid = isVideoWallpaper(url);
                    const thumb = isVid
                        ? `<video src="${url}" muted playsinline style="width:100%;height:100%;object-fit:cover;pointer-events:none;"></video>`
                        : `<div style="background-image:url('${url}');width:100%;height:100%;background-size:cover;background-position:center;"></div>`;
                    return `<div class="wall-grid-item ${i === State.currentWall ? 'active' : ''}" data-wall-idx="${i}" onclick="Apps.settings.previewWall(${i}, this)" style="position:relative;min-width:180px;width:180px;height:320px;border-radius:18px;overflow:hidden;border:2.5px solid ${i === State.currentWall ? 'var(--accent)' : 'transparent'};cursor:pointer;flex-shrink:0;scroll-snap-align:center;transition:border-color 0.3s ease, transform 0.3s ease, opacity 0.3s ease;">${showSparkle ? sparkle(i) : ''}${thumb}<div class="wall-apply-btn" onclick="Apps.settings.applyWall(${i}); event.stopPropagation()">Apply</div></div>`;
                };

                let customContent = '';
                if (customWalls.length > 0) {
                    customContent = customWalls.map(w => wallItem(w.url, w.i, false)).join('');
                    customContent += `<div class="wall-grid-item" onclick="document.getElementById('wall-input').click()" style="min-width:180px;width:180px;height:320px;border-radius:18px;overflow:hidden;border:2px dashed #444;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;scroll-snap-align:center;transition:opacity 0.3s ease;"><i class="fas fa-plus" style="color:var(--text-sec);font-size:32px;"></i></div>`;
                } else {
                    customContent = `<div onclick="document.getElementById('wall-input').click()" style="min-width:180px;width:180px;height:320px;border-radius:18px;border:2px dashed #444;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;scroll-snap-align:center;flex-direction:column;gap:10px;transition:opacity 0.3s ease;"><i class="fas fa-plus" style="color:var(--text-sec);font-size:32px;"></i><span style="color:var(--text-sec);font-size:14px;">Add wallpaper</span></div>`;
                }

                const makeRow = (title, items, showToggle) => {
                    return `<div style="background:var(--bg-card);border-radius:14px;padding:14px;margin-bottom:12px;">
                        <div style="font-size:16px;font-weight:600;color:var(--text-main);margin-bottom:12px;padding-left:10px;">${title}</div>
                        <div class="wall-carousel" style="display:flex;gap:20px;overflow-x:auto;padding:10px calc(50% - 90px);flex-wrap:nowrap;scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch;scroll-padding:0 calc(50% - 90px);">
                            ${items}
                        </div>
                        ${showToggle ? `<div class="special-fx-toggle" id="toggle-${title.replace(/\s+/g, '')}" style="margin-top:12px;overflow:hidden;transition:max-height 0.35s ease, opacity 0.35s ease, filter 0.3s ease;max-height:0;opacity:0;">
                            <div class="list-item" onclick="Apps.settings.toggleSpecialEffects()" style="background:rgba(128,128,128,0.08);border-radius:10px;padding:12px 14px;">
                                <span>Special Effects</span>
                                <div class="toggle ${State.specialEffects ? 'active' : ''}"></div>
                            </div>
                            <div style="margin:6px 14px 0;font-size:12px;color:var(--text-sec);">Enables special effects for this wallpaper</div>
                        </div>` : ''}
                    </div>`;
                };

                content = `
                    <div class="anim-fade" style="padding:0;height:100%;box-sizing:border-box;overflow-y:auto;overflow-x:hidden;">
                        <div class="list-group" style="margin:16px 16px 20px;">
                        ...
                        ${makeRow('Custom Wallpapers', customContent, false)}
                        ${makeRow('Default Wallpapers', defaultWalls.map(w => wallItem(w.url, w.i, false)).join(''), false)}
                        ${makeRow('Special Wallpapers', specialWalls.map(w => wallItem(w.url, w.i, true)).join(''), true)}
                        ${makeRow('Xiaomi Wallpapers', xiaomiWalls.map(w => wallItem(w.url, w.i, isVideoWallpaper(w.url))).join(''), true)}
                        ${makeRow('OriginOS Wallpapers', originWalls.map(w => wallItem(w.url, w.i, false)).join(''), false)}
                    </div>`;
                setTimeout(() => {
                    const wallInput = document.getElementById('wall-input');
                    if (wallInput) wallInput.onchange = (e) => {
                        ...
                        if (f) {
                            ...
                            r.onload = (ev) => {
                                ...
                                Storage.saveSettings();
                                OS.applySettings();
                                Apps.settings.render('wallpaper');
                            };
                            r.readAsDataURL(f);
                        }
                    };

                    const currentEl = document.querySelector(`.wall-grid-item[data-wall-idx="${State.currentWall}"]`);
                    if (currentEl) {
                        const btn = currentEl.querySelector('.wall-apply-btn');
                        if (btn) {
                    ...
                    const updateOpacity = (carousel) => {
                        const cRect = carousel.getBoundingClientRect();
                        const cCenter = cRect.left + cRect.width / 2;
                        carousel.querySelectorAll('.wall-grid-item, [onclick*="wall-input"]').forEach(item => {
                            const iRect = item.getBoundingClientRect();
                            const iCenter = iRect.left + iRect.width / 2;
                            const diff = Math.abs(cCenter - iCenter);
                            const maxDist = 200;
                            let op = 1 - (diff / maxDist);
                            if (op < 0.4) op = 0.4;
                            if (op > 1) op = 1;
                    ...
                    document.querySelectorAll('.wall-carousel').forEach((carousel, i) => {
                        if (savedScrolls[i] !== undefined) {
                            carousel.scrollLeft = savedScrolls[i];
                        }
                ...
                content = `<div class="anim-fade" style="padding-top:90px; position:relative; min-height:100%;">
                            ...
                            <div class="about-hero" style="z-index:2; position:relative;">
                                ...
                                <div class="realos-ver">V2.3.0</div>
                            </div>
            ...
            if (view !== 'root' && view !== 'about' && content) {
                const inlineHeader = `<div class="settings-inline-header"><span class="settings-inline-back" onclick="Apps.settings.render('root')"><i class="fas fa-chevron-left"></i> Settings</span><span class="settings-inline-title">${headerTitle.innerText}</span><span style="width:40px"></span></div>`;
        ...
        previewWall: (i, el, fromScroll) => {
            ...
            if (el) {
                ...
                if (btn) {
                    const isApplied = (i === State.currentWall);
                    btn.innerText = isApplied ? "Applied" : "Apply";
                    btn.style.background = isApplied ? "#000" : "var(--accent)";
                    btn.style.opacity = '1';
                    btn.style.pointerEvents = 'auto';
                }
        ...
        applyWall: (i) => {
            ...
            const el = document.querySelector(`.wall-grid-item[data-wall-idx="${i}"]`);
            if (el) {
                const btn = el.querySelector('.wall-apply-btn');
                if (btn) {
                    btn.innerText = "Applied";
        ...
        setWall: (i) => {
            const el = document.querySelector(`.wall-grid-item[data-wall-idx="${i}"]`);
            Apps.settings.previewWall(i, el);
        },
        toggleAOD: () => {
